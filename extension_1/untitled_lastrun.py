...
urndlist.append(1)
uprize_lst=[(-10,10),(-10,100),(10,10),(10,100)]
...
rndlist.append(1)
prize_lst=[(-10,10),(-10,100),(10,10),(10,100)]
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\hp\\OneDrive\\Desktop\\banditext1\\untitled_lastrun.py',
...
uIntrotext = visual.TextStim(win=win, name='uIntrotext',
    text='Welcome!\n\nIn this part of the task you are initially awarded . You will have unlimited time to select an option (Q/W/O/P). Each option corresponds to a penalty which will be deducted from your initial award.\n\nYour goal is to maximize your final award by minimising the penalties.\n\nPress\xa0SPACE\xa0to\xa0begin',
    font='Open Sans',
...
text = visual.TextStim(win=win, name='text',
    text='Welcome! \n\nIn this part of the task you are initially awarded 200000. \nYou will have limited time (0.4s) to select an option (Q/W/O/P).\nEach option corresponds to a penalty which will be deducted from your initial award. Incase you fail to select an option before the timer runs out, a penalty amount will be deducted.\n\nYour goal is to maximize your final award by minimising the penalties.\n\nPress\xa0SPACE\xa0to\xa0begin',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='black', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
clk = visual.ImageStim(
    win=win,
    name='clk', 
    image='clck.jpg', mask=None, anchor='center',
    ori=0.0, pos=(0, 0.15), size=(0.3, 0.3),
    color=[1,1,1], colorSpace='rgb', opacity=0.5,
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=0.0)
...
utrials = data.TrialHandler(nReps=400.0, method='random', 
...
for thisUtrial in utrials:
    ...
    if ukey_resp.keys=='q':
        uprize=random.gauss(uprize_lst[0][0],uprize_lst[0][1])
        uprize+=30 + (random.random() * (30))
    elif ukey_resp.keys=='w':
        uprize=random.gauss(uprize_lst[1][0],uprize_lst[1][1])
        uprize+=30 + (random.random() * (30))
    elif ukey_resp.keys=='o':
        uprize=random.gauss(uprize_lst[2][0],uprize_lst[2][1])
        uprize+=30 + (random.random() * (30))
    elif ukey_resp.keys=='p':
        uprize=random.gauss(uprize_lst[3][0],uprize_lst[3][1])
        uprize+=30 + (random.random() * (30))
    else:
        uprize=0
    uprize=max(uprize,0)
    ...
    routineForceEnded = False
    # update component parameters for each repeat
    text_3.setText('Loss -' + str(round(uprize,0)) )
    # keep track of which components have finished
    uresultComponents = [sad1, text_3]
    ...
    while continueRoutine and routineTimer.getTime() < 0.4:
        ...
        # *sad1* updates
        if sad1.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            sad1.frameNStart = frameN  # exact frame index
            sad1.tStart = t  # local t and not account for scr refresh
            sad1.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(sad1, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'sad1.started')
            sad1.setAutoDraw(True)
        if sad1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > sad1.tStartRefresh + 0.4-frameTolerance:
                # keep track of stop time/frame for later
                sad1.tStop = t  # not accounting for scr refresh
                sad1.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'sad1.stopped')
                sad1.setAutoDraw(False)
    ...
    for thisTrial_2 in trials_2:
        ...
        routineForceEnded = False
        # update component parameters for each repeat
        text_8.setText('Your current balance is ' + str(round(total,0)) + '\n\nYou can now rest for 30s and you will again get 10000 \n \n Press SPACE to continue')
        key_resp_5.keys = []
        key_resp_5.rt = []
        _key_resp_5_allKeys = []
        # keep track of which components have finished
...
for thisTrial in trials:
    ...
    if key_resp.keys=='q':
        prize=random.gauss(prize_lst[0][0],prize_lst[0][1])
        prize+=30 + (random.random() * (30))
        pos=0.4
        neg=0
        cross=0
    elif key_resp.keys=='w':
        prize=random.gauss(prize_lst[1][0],prize_lst[1][1])
        prize+=30 + (random.random() * (30))
        pos=0.4
        neg=0
        cross=0
    elif key_resp.keys=='o':
        prize=random.gauss(prize_lst[2][0],prize_lst[2][1])
        prize+=30 + (random.random() * (30))
        pos=0.4
        neg=0
        cross=0
    elif key_resp.keys=='p':
        prize=random.gauss(prize_lst[3][0],prize_lst[3][1])
        prize+=30 + (random.random() * (30))
        pos=0.4
        neg=0
        cross=0
    ...
    routineForceEnded = False
    # update component parameters for each repeat
    text_2.setText('Loss -' + str(round(prize,0)))
    # keep track of which components have finished
    resultComponents = [clk, sad, text_2]
    ...
    while continueRoutine:
        ...
        # *clk* updates
        if clk.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            clk.frameNStart = frameN  # exact frame index
            clk.tStart = t  # local t and not account for scr refresh
            clk.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(clk, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'clk.started')
            clk.setAutoDraw(True)
        if clk.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > clk.tStartRefresh + neg-frameTolerance:
                # keep track of stop time/frame for later
                clk.tStop = t  # not accounting for scr refresh
                clk.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'clk.stopped')
                clk.setAutoDraw(False)
        ...
        if sad.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'sad.started')
            sad.setAutoDraw(True)
        if sad.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > sad.tStartRefresh + pos-frameTolerance:
                # keep track of stop time/frame for later
                sad.tStop = t  # not accounting for scr refresh
                sad.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'sad.stopped')
    ...
    if(cntrnd==20):
        ...
        if(rndtype==2):#equalmeans
            prize_lst=[ (0,10), (0,40), (0,70), (0,100)]
        if(rndtype==3):#lowvar
            prize_lst=[ (-10,10), (-0.3333,10), (0.3333,10), (10,10)]
        if(rndtype==4):#highvar
            prize_lst=[ (-10,100), (-0.3333,100), (0.3333,100), (10,100)]
    ...
    for thisTrial_3 in trials_3:
        ...
        routineForceEnded = False
        # update component parameters for each repeat
        text_9.setText('Your current balance is ' + str(round(total,0)) + '\n\nYou can now rest for 30s and you will again get 10000\n \n Press SPACE to continue')
        key_resp_4.keys = []
        key_resp_4.rt = []
        _key_resp_4_allKeys = []
        # keep track of which components have finished
...
routineForceEnded = False
# update component parameters for each repeat
text_4.setText('Total Reward '+ str(round(ftotal,0))+'\n\nThank You for participating!'
)
# keep track of which components have finished
