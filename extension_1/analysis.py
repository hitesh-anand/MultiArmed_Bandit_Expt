...
for ind, row in enumerate(data_untimed):
    untimed_values[conditions[row[1]]][int(row[2]/2)] += row[3]
...
for ind, row in enumerate(data_timed):
    timed_values[conditions[row[1]]][int(row[2]/2)] += row[3]
    timed_count[conditions[row[1]]][int(row[2]/2)] += 1

init = [init_rew for i in range(11)]

for i in range(4):
    y_vals_untimed = []
    y_vals_timed = []
    for j in range(11):
        y_vals_untimed.append(init[j] - untimed_values[i][j]/untimed_count[i][j])
        y_vals_timed.append(init[j] - timed_values[i][j]/timed_count[i][j])

    plt.subplot(2, 2, i+1)
    if i==0:
        plt.title("Amount remaining after each Trial (in IOWA)")
    elif i==1:
        plt.title("Amount remaining after each Trial (in Equal Means)")
    elif i==2:
        plt.title("Amount remaining after each Trial (in Low Var)")
    elif i==3:
        plt.title("Amount remaining after each Trial (in High Var)")
    ...
    plt.yticks([9000+i for i in range(0, 1000, 200)])
    plt.tight_layout()
    plt.legend(['untimed', 'timed'])
