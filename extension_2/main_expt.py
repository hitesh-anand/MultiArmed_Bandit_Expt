...
newtrials = data.TrialHandler(nReps=240.0, method='random', 
...
#        newprize+=30 + (random.random() * (30))
        ...
        newprize=shift + random.gauss(newprize_lst[3][0],newprize_lst[3][1])
#        newprize+=30 + (random.random() * (30))
    else:
        newprize=0
    ...
    for thisTrial_2 in trials_2:
        ...
        routineForceEnded = False
        # update component parameters for each repeat
        text_2.setText('Your total reward based on your performance till now is: ' + str(round(ftot))  )
        # keep track of which components have finished
    ...
    if(newcntrnd%2==0):
        ...
        file.write(str(expInfo['Roll Number'])+", "+conditions[str(newrndtype)]+", "+str(newcntrnd)+", "+str(tot)+", "+str(rep_choices)+"\n")
        file.close()
    newcntrnd+=1
    if(newcntrnd==20):
        history=[]
